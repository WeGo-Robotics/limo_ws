...
class Class_Name:  # 클래스 1단계 : 클래스 이름
    def __init__(self):  # 클래스 2단계 : 클래스 초기화 및 초기 설정
        ...
        self.pub = rospy.Publisher("/counter", Int32, queue_size=1)  # ROS 2단계(필수) : 노드 역할 - 퍼블리셔 설정
        self.rate = rospy.Rate(10)  # ROS 2-1단계(옵션) : 퍼블리셔 - 주기 설정
        self.msg = Int32()  # 메세지 타입 설정 및 초기화

    def func(self):  # 클래스 3단계 : 함수 설정
        self.pub.publish(self.msg)  # ROS 3단계(필수) : 퍼블리셔 - 퍼블리시 실행
        print(f"msg:{self.msg}")  # 출력 : 메세지
        print(f"msg.data:{self.msg.data}")  # 출력 : 메세지 항목(데이터)
        self.msg.data += 1  # 메세지 항목(데이터) - 데이터 값 변경
